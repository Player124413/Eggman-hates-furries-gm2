...
if ((y>=objWaterFront.y and yp<objWaterFront.y) or (y<objWaterFront.y and yp>=objWaterFront.y))
    ...
    i.hspeed=(hspeed+objWaterFront.hspeed*0.25);
    }
yp=y;
...
if(panjan.phase==3 && panjan.subphs<2)
    {
...
__b__ = action_if(point_distance(x+hspeed*global.time,y+vspeed*global.time,sonic.x,sonic.y)<18+16 && sonic.able && invulnerable==0 && panjan.phase!=3);
if __b__
{
if(shield)
    {
    if(sonic.invulnerable==0)
        {
        sonic.damaged=1;
        sonic.damagex=sonic.x-8;
        sonic.damagey=sonic.y;
        }
    }
else
    {
